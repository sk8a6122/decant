...
const phrases:Record<string,string[]>={
 'app.intensity':['Only a faint wash of colour shows.','The colour is neither faint nor dense.','The colour looks dense and strongly saturated.'],
 'app.colour':[''],
 ...
 'pal.finish':['The flavour disappears quickly after swallowing.','The flavour lingers briefly, a little beyond the shortest finish.','The flavour lasts a moderate time after swallowing.','The flavour lingers quite a while, though not at the longest end.','The flavour stays with me for a long time after swallowing.']
};
const colours:Record<string,string>={purple:'violet-red',ruby:'bright jewel-red',garnet:'red with a rusty edge',tawny:'orange-brown',brown:'brown',pink:'pink',salmon:'orange-pink',orange:'orange','lemon-green':'yellow with a green tinge',lemon:'plain yellow',gold:'a rich golden yellow',amber:'an orange-yellow amber shade'};
export function noteExercise(ref:WineReference){
 const evidence:Record<string,string>={};
 for(const [key,options] of Object.entries(phrases)){
  if(key==='app.colour'){evidence[key]=`Its hue is ${colours[String(ref.grid[key])]}.`;continue;}
 ...
 evidence['pal.flavours']=`On my tongue I find ${ref.flavours.join(', ')}.`;
