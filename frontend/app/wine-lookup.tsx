...
export function WineLookup({onSelect}:{onSelect:(wine:any,vintage:string|null)=>void}){
 ...
 return <section className="wine-lookup wide" aria-label="Find a wine">
  ...
  <ul id={listId} role="listbox" aria-label="Wine matches" className="wine-results" hidden={!isOpen}>{isOpen&&items.map((wine:any,i:number)=><li role="option" id={`${listId}-${i}`} aria-selected={active===i} key={wine.lwin} onMouseDown={e=>e.preventDefault()} onClick={()=>select(wine)}><strong>{wine.displayName}</strong><span>{[wine.country,wine.region,wine.colour].filter(Boolean).join(' · ')} · LWIN {wine.lwin}</span></li>)}</ul>
