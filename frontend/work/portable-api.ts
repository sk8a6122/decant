...
if(path==='import'){if(!Array.isArray(body.bottles)||!Array.isArray(body.notes))throw new Error('Choose a Decant notebook JSON export.');s.records=importEntries(s.records,body);if(body.academy)s.academy=mergeAcademy(s.academy,body.academy);if(Array.isArray(body.education))s.content=body.education;if(Array.isArray(body.progress))s.progress=body.progress;save(s);return {saved:true}}
