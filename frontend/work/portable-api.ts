...
function load(){const raw=localStorage.getItem(key);const s=raw?JSON.parse(raw):{name:'',records:[],progress:[],content:initialContent};if(s.schemaVersion!==3){s.records=linkLegacyNotes(s.records);s.schemaVersion=3;save(s);}return s;}
...
const s=load();
if(path==='bootstrap')return {member:{id:'local-notebook',email:'Stored only in this browser',name:s.name,admin:new URLSearchParams(location.search).get('studio')==='1',tier:3,status:'local'},records:s.records,progress:s.progress,content:s.content.map((c:any)=>({...c,locked:false,lessonCount:c.body.length})),plans,billingReady:false,launch:null};
...
if(path==='import'){if(!Array.isArray(body.bottles)||!Array.isArray(body.notes))throw new Error('Choose a Decant notebook JSON export.');s.records=importEntries(s.records,body);if(Array.isArray(body.education))s.content=body.education;if(Array.isArray(body.progress))s.progress=body.progress;save(s);return {saved:true}}
