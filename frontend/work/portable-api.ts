...
if(path==='bootstrap')return {member:{id:'local-notebook',email:'Stored only in this browser',name:s.name,currency:s.currency||'USD',admin:new URLSearchParams(location.search).get('studio')==='1',tier:3,status:'local'},records:s.records,academy:s.academy,progress:s.progress,content:s.content.map((c:any)=>({...c,locked:false,lessonCount:c.body.length})),plans,billingReady:false,launch:null};
...
if(path==='import'){if(!Array.isArray(body.bottles)||!Array.isArray(body.notes))throw new Error('Choose a Decant notebook JSON export.');s.records=importEntries(s.records,body);if(body.preferences?.currency)s.currency=currencyCode(body.preferences.currency);if(body.academy)s.academy=mergeAcademy(s.academy,body.academy);if(Array.isArray(body.education))s.content=body.education;if(Array.isArray(body.progress))s.progress=body.progress;save(s);return {saved:true}}
