...
test('vintage candidates must be clear standalone lines, not historical prose',()=>{const parsed=labelReading(data([['TIGNANELLO',94,55],['ANTINORI',93,35],['2021',95,28],['First vintage 1971',95,10]]));assert.equal(parsed.query,'TIGNANELLO ANTINORI');assert.deepEqual(parsed.years,['2021']);assert.equal(parsed.vintage,undefined)});
